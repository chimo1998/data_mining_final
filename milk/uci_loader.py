...
class Loader(object):
    ...
    def read_from_url(self):
        self.data = pd.read_csv(os.path.join(os.getcwd(),self.url), names=self.names)
    ...
    def get_data(self):
        train = self.data[self.data['10'].notnull()]
        train['9'].fillna(0, inplace=True)
        test = self.data[self.data['10'].isnull()]
